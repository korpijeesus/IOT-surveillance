...
#Importataan MySQL kirjasto
import MySQLdb

from datetime import datetime
from PIL import Image

#MySQL yhdistäminen ja kursorin luominen
conn = MySQLdb.connect(host= "localhost", user="root", passwd="Salasana123", db="iot")
cursor = conn.cursor()
...
while (True):
    ...
    for x in xrange(0, 100):
        ...
        if changedPixels > sensitivity:
            ...
            cursor.execute("INSERT INTO log VALUES (now.year, now.month, now.day, now.hour, now.minute, now.second)")
            conn.commit()
